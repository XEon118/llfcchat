...
void ApplyFriend::addLabel(QString name)
{
    ...
    _label_point.setX(_label_point.x() + tmplabel->width() + 2);    //计算最新位置
    //换行逻辑
    if (_label_point.x() + MIN_APPLY_LABEL_ED_LEN > ui->gridWidget->width()) {
...
void ApplyFriend::SlotAddFirendLabelByClickTip(QString text)
{
    ...
    //标签展示栏没有该标签，标签展示栏也增加一个标签, 并设置绿色选中
    auto* lb = new ClickedLabel(ui->lb_list);
    lb->SetState("normal", "hover", "pressed", "selected_normal",
                 "selected_hover", "selected_pressed");
    lb->setObjectName("tipslb");
...
void ApplyFriend::SlotApplySure()
{
    ...
    QJsonObject jsonObj;
    auto uid = UserMgr::GetInstance()->GetUid();    //获取自己的uid
    jsonObj["uid"] = uid;
    auto name = ui->name_ed->text();    //获取自己的名字
    if(name.isEmpty()){
        name = ui->name_ed->placeholderText();  //名字为空，用默认
    }
    jsonObj["applyname"] = name;
    auto bakname = ui->back_ed->text();    //备注名
    if(bakname.isEmpty()){
        bakname = ui->back_ed->placeholderText();  //名字为空，用默认
    }
    jsonObj["bakname"] = bakname;
    jsonObj["touid"] =  _si->_uid;    //要申请好友的uid

    QJsonDocument doc(jsonObj);
    QByteArray jsonData = doc.toJson(QJsonDocument::Compact);   //压缩方式

    //发送tcp请求给chatserver
    emit TcpMgr::GetInstance()->sig_send_data(ReqId::ID_ADD_FRIEND_REQ,jsonData);  //发送添加好友请求
