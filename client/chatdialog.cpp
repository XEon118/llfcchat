    ...
    _state(ChatUIMode::ChatMode),_b_loading(false),_cur_chat_uid(0),_last_widget(nullptr)
{
    ...
    ui->search_edit->addAction(searchAction,QLineEdit::LeadingPosition);    //头部位置
    ui->search_edit->setPlaceholderText(QStringLiteral("搜索"));  //默认文本
    ...
    addChatUserList();  //添加用于测试的chat_user
...
void ChatDialog::addChatUserList()
{
    ...
    //模拟 ： 创建QListWidgetItem，并设置自定义的widget
    for(int i = 0; i < 13; i++){
        ...
        item->setSizeHint(chat_user_wid->sizeHint());
        ui->chat_user_list->addItem(item);
        ui->chat_user_list->setItemWidget(item, chat_user_wid); //设置成chat_user_wid样式
    }
}
...
void ChatDialog::slot_apply_friend(std::shared_ptr<AddFriendApply> apply)
{
    ...
    ui->side_contact_lb->ShowRedPoint(true);    //红点显示
    ui->con_user_list->ShowRedPoint(true);
    ui->friend_apply_page->AddNewApply(apply);  //添加新的请求
}
...
void ChatDialog::slot_add_auth_friend(std::shared_ptr<AuthInfo> auth_info)
{
    ...
    ui->chat_user_list->setItemWidget(item, chat_user_wid);
    _chat_items_added.insert(auth_info->_uid, item);    //添加聊天条目
}
...
void ChatDialog::slot_auth_rsp(std::shared_ptr<AuthRsp> auth_rsp)
{
    ...
    ui->chat_user_list->insertItem(0, item);
    ui->chat_user_list->setItemWidget(item, chat_user_wid);
    _chat_items_added.insert(auth_rsp->_uid, item);     //添加聊天条目
}
...
void ChatDialog::slot_jump_chat_item(std::shared_ptr<SearchInfo> si)
{
    ...
    if(find_iter != _chat_items_added.end()){
        ...
        ui->side_chat_lb->SetSelected(true);
        SetSelectChatItem(si->_uid);    //选中状态
        //更新聊天界面信息
...
void ChatDialog::slot_text_chat_msg(std::shared_ptr<TextChatMsg> msg)
{
    ...
    if(find_iter != _chat_items_added.end()){
        ...
        chat_wid->updateLastMsg(msg->_chat_msgs);   //更新最后的消息
        //更新当前聊天页面记录
