...
{
    ui->setupUi(this);
    ui->pass_edit->setEchoMode(QLineEdit::Password); //密码模式，保护隐私
    ui->confirm_edit->setEchoMode(QLineEdit::Password);
    ui->err_tip->setProperty("state","normal"); //设置属性，才会在qss中寻找
    repolish(ui->err_tip);
...
void RegisterDialog::slot_reg_mod_finish(ReqId id, QString res, ErrorCodes err)
{
    ...
    _handlers[id](jsonDoc.object()); //通过id调用找到并传参，返回的就是initHttpHandlers中的lambda表达式，做的事也在表达式中
    return;
}
