...
private:
    void addContactUserList();  //添加用户列表

public slots:
    void slot_item_clicked(QListWidgetItem *item);
    void slot_add_auth_firend(std::shared_ptr<AuthInfo>);   //被同意方，收到同意好友请求槽函数
    void slot_auth_rsp(std::shared_ptr<AuthRsp>);       //同意方，同意好友后回包槽函数
signals:
