...
TcpMgr::TcpMgr():_host(""),_port(0),_b_recv_pending(false),_message_id(0),_message_len(0)
{
    ...
    QObject::connect(&_socket, &QTcpSocket::readyRead, [&]() {
        ...
        forever {
            ...
            // 读取消息体
            QByteArray messageBody = _buffer.mid(0, _message_len);  //截取
            qDebug() << "receive body msg is " << messageBody ;

            _buffer = _buffer.mid(_message_len);    //截掉buffer便于处理下一条msg
...
{
    //auto self = shared_from_this();   还没构造完，所以不能分享
    //处理登录回包
    _handlers.insert(ID_CHAT_LOGIN_RSP, [this](ReqId id, int len, QByteArray data){
        ...
        //UserMgr类统一管理用户
        UserMgr::GetInstance()->SetUserInfo(user_info); //设置用户基本信息
        UserMgr::GetInstance()->SetToken(jsonObj["token"].toString()); //保留token字段
        ...
        emit sig_switch_chatdlg();   //跳转到聊天界面
    });
    //处理搜索好友回包
    _handlers.insert(ID_SEARCH_USER_RSP, [this](ReqId id, int len, QByteArray data){
        ...
        auto search_info = std::make_shared<SearchInfo>(jsonObj["uid"].toInt(),
                                                        ...
                                                        jsonObj["desc"].toString(),jsonObj["sex"].toInt(),
    ...
    _handlers.insert(ID_NOTIFY_ADD_FRIEND_REQ, [this](ReqId id, int len, QByteArray data) {
        ...
        if (!jsonObj.contains("error")) {
            int err = ErrorCodes::ERR_JSON;
            qDebug() << "NOTIFY ADD FRIEND REQ Failed, err is Json Parse Err" << err;
            emit sig_user_search(nullptr);
            return;
        }
        ...
        int err = jsonObj["error"].toInt();
        if (err != ErrorCodes::SUCCESS) {
            qDebug() << "NOTIFY ADD FRIEND REQ Failed, err is " << err;
            emit sig_user_search(nullptr);
            return;
        }
        //解析
