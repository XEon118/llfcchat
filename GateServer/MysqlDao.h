...
public:
	...
	std::unique_ptr<sql::Connection> _con;	//自动回收con
	...
	MySqlPool(const std::string& url, const std::string& user, const std::string& pass, const std::string& schema, int poolSize)
		...
		try {
			...
			_check_thread = std::thread([this]() {
				while (!b_stop_) {
					checkConnectionPro();
					std::this_thread::sleep_for(std::chrono::seconds(60));	//每隔60s检测
				}
				});

			_check_thread.detach();	//后台分离式运行
		}
...
private:
	...
	std::mutex mutex_;	//防止多线程访问保证队列安全
	std::condition_variable cond_;	//队列为空的条件变量，访问线程挂起
	std::atomic<bool> b_stop_;
	std::thread _check_thread;	//检测线程，心跳
